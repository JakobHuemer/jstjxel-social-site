...
function parseMessage(message, logger) {
    ...
    parsedMessage.command = parseCommand(rawCommandComponent, logger);
    ...
    if (null == parsedMessage.command) {  // Is null if it's a message we don't care about.
        ...
        if (null != rawTagsComponent) {  // The IRC message contains tags.
            parsedMessage.tags = parseTags(rawTagsComponent, logger);
        }

        parsedMessage.source = parseSource(rawSourceComponent, logger);

        parsedMessage.parameters = rawParametersComponent;
        if (rawParametersComponent && rawParametersComponent[0] === '!') {
            // The user entered a bot command in the chat window.
            parsedMessage.command = parseParameters(rawParametersComponent, parsedMessage.command, logger);
        }
...
function parseTags(tags, logger) {
...
function parseCommand(rawCommandComponent, logger) {
    ...
    switch (commandParts[0]) {
        ...
        case 'CAP':
            parsedCommand = {
                command: commandParts[0], isCapRequestEnabled: (commandParts[2] === 'ACK'), // The parameters part of the messages contains the
        ...
        case 'ROOMSTATE':   // But it has no meaning without also including the /tags capabilities.
            ...
            break;
        case 'RECONNECT':
            logger.warn('The Twitch IRC server is about to terminate the webSocketMessageServerConnection for maintenance.', 'IRC PARSER');
            parsedCommand = {
                command: commandParts[0]
            };
            break;
        case '421':
            logger.error(`Unsupported IRC command: ${ commandParts[2] }`, 'IRC PARSER');
            return null;
        ...
        case '376':
            ...
            logger.log(`numeric message: ${ commandParts[0] }`, 'IRC PARSER');
            return null;
        default:
...
function parseSource(rawSourceComponent, logger) {
...
function parseParameters(rawParametersComponent, command, logger) {
    let idx = 0;
...
export { parseMessage, parseTags, parseCommand, parseSource, parseParameters };